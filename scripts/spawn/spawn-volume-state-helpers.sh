...
function conditionally_mark_volume_is_necessary(){
  ...
  report_action_taken_to_log "Set state to mark that volume “${volume_name}” must exist${NEWLINE}and be encrypted using 1Password item key “${op_item_key}”."
...
function construct_state_string_for_volume_1password_key_pending_creation(){
  ...
  #
  # With --volume-only, constructs a truncated version of the state string where the trailing 
