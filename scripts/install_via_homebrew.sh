...
#   op read 'op://…/notesPlain' | sudo ./provision_vols_and_users.sh --stdin-json
#
# JSON shape (example):
# {
...
# Behavior (idempotent):
#   - If a volume with the same name already exists in the container: no-op
#   - Otherwise, create encrypted APFS volume with provided passphrase
...
function create_local_user_account() {
  ...
  if [[ -n "${avatar_rel}" ]]; then
    ...
    if [[ -f "$avatar_abs" ]]; then
      ...
      report "  - Avatar not found at '$avatar_abs' (skipping)"
    fi
  else
    report "  - No avatar filename provided; skipping"
  fi
}
